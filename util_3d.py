...
def export_instance_ids_for_eval(filename, label_ids, instance_ids, json_name):
    ...
    with open(filename, 'w') as f:
        for idx, inst_id in enumerate(insts):
            if inst_id == 0:  # 0 -> no instance for this vertex
                continue
            output_mask_file = os.path.join(output_mask_path_relative, name + '_' + str(idx-1) + '.txt')
            loc = np.where(instance_ids == inst_id)
            label_id = label_ids[loc[0][0]]
            id_to_label[str(idx-1)] = str(label_id)
        ...
        with open(json_name, "w") as j:
            j.write(json.dumps(id_to_label))
